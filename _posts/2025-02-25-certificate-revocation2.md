...
Following on from [part 1](/posts/certificate-revocation1/), which covered the formative CRL and OCSP approaches, this post explores the revocation approaches that have introduced a return to the use of CRLs — browser vendors distributing revocation lists through alternative methods.
...
- Microsoft Edge: disallowedcert.stl
...
This will always be a trade-off (as the guaranteed option of asking the CA for every connection clearly undoes the distributed nature of PKI/public key signing  - we may as well use Kerberos if we are pursuing a centralised model) however the mechanisms outlined in these posts ultimately just shorten this time window (e.g. through periodic OCSP acquisition, or periodic generation of distributed CRLs in some form to clients).  

If we're going to this effort anyway, shorter lived certificates seem far (far) simpler.

**Update (2025):** The CA/B Forum has since acted on this direction. Ballot SC-081, passed in March 2025, mandates reducing maximum TLS certificate lifetimes from 398 days to 47 days by 2027 — effectively enshrining short-lived certificates as the WebPKI standard.

## Summary

- Browser vendors have developed their own mechanisms to distribute revocation information, motivated by the unreliability and performance costs of client-side CRL/OCSP checking.
- Mozilla uses two approaches: OneCRL for intermediate certificate revocations (~1600 entries distributed via Firefox RemoteSettings) and CRLite for end-entity revocations — a bloom filter covering all non-expired revoked WebPKI certificates, updated several times daily via delta files.
- Google Chrome uses CRLsets (a selective list of ~840 high-impact revocations distributed via the Component Updater) and a blocklist (certificates revoked due to major CA incidents, shipped with the browser).
- All of these mechanisms work by aggregating CA-published CRL data via CCADB and Certificate Transparency logs, then distributing processed summaries to browsers — avoiding the need for clients to download raw CRLs or query OCSP responders directly.
- The complexity of these mechanisms reinforces the case for short-lived certificates. The CA/B Forum Ballot SC-081 (March 2025) mandated reducing maximum TLS certificate lifetimes from 398 days to 47 days by 2027, directly acting on this direction.
