...
Having worked through certificates in other posts, I've accepted that certificate signatures *work* — without looking at how they are generated and validated.

This post explores RSA certificate signing and verification — specifically the `sha256WithRSAEncryption` algorithm, which openssl used by default when I created the dummyCA in [this post](/posts/what-makes-an-intermediate-certificate-intermediate/).

This [blog post](https://www.keenformatics.com/manually-verifying-an-ssl-certificate/) was invaluable in grasping the signature verification mechanism — I replicate much of it here, but use Python throughout and a dummyCA to explore the signing process.
...
- can I re-create the signing and verification processes by hand, down to the mathematical operations?
...
I'll work through the signing process and recreate this arithmetic operation to arrive at the same signature value openssl generated.
...
First, import the relevant modules, load the CA certificate, and create a cert object (error handling omitted for brevity).
...
The message is not solely the sha256 hash output as there are security property reasons which require the encoded value to be close to the size of the modulus.  This is where [section 8.2](https://datatracker.ietf.org/doc/html/rfc8017#section-8.2) of rfc8017 comes in.

### The PKCS #1 V1.5 encoding scheme

RFC8017 describes encoding the message digest (the tbs_certificate_bytes value) into an ASN.1 value - modules exist for parsing of asn1 however the notes section of rfc8017 provides a shortcut that avoids this complexity entirely:
...
- The signing algorithm chain for `sha256WithRSAEncryption` runs from RFC5280 → RFC4055 → RFC8017, with RFC8017 section 8.2 and 9.2 defining the specific steps.
- Only the *to-be-signed* (tbs) section of a certificate is signed — the outer signature algorithm identifier and signature value fields are excluded.
- The hash of the tbs section is not used directly as the message. It is first encoded using the PKCS #1 v1.5 scheme (RFC8017 section 9.2), padding the value to match the key size.
- Signing is the modular exponentiation $$signature = EM^{d}\bmod{N}$$, where $$d$$ is the private key exponent and $$N$$ is the RSA modulus.
- Verification is the inverse operation $$EM = signature^{e}\bmod{N}$$, using only the public exponent $$e$$ and modulus $$N$$ from the certificate — no private key required.
- Both operations were recreated in Python and confirmed to match the values generated and verified by openssl.