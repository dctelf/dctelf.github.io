...
This post covers the webPKI model of a client authenticating a service — background context for a later post on mTLS.
...
Service authentication practices have evolved since TLS's early days and are now articulated in [RFC9525](https://datatracker.ietf.org/doc/html/rfc9525), with [section 6](https://datatracker.ietf.org/doc/html/rfc9525#name-verifying-service-identity) covering the key themes of service identity verification.
...
1. **CA root certificate creation:** A certificate authority generates its keypair and issues a root (self signed) certificate
2. **WebPKI acceptance and distribution:** OS & Browser vendors trust this CA and are satisfied with its processes/controls - they agree to distribute the CA certificate to the clients they control/support
3. **Client trust store inclusion:** The CA certificate is installed within the clients truststore (e.g from creation of the original installation media by the vendor, or through an update etc.)
...
7. **Signed certificate installation:** The service *installs* the signed certificate (/bundle) on its service (e.g. on a webserver, or loadbalancer etc.).
...
> One aspect the flow above omits: the service must also prove it holds the private key corresponding to the presented certificate. During the TLS handshake, the service performs a cryptographic operation using its private key that the client can verify against the public key in the certificate — this is what binds the certificate to the actual service, rather than just confirming the certificate is valid. A later post will explore the handshake mechanics in detail, including how this becomes a two-way proof in mTLS where both parties must demonstrate private key possession.
{: .prompt-info }

## Certificate properties

As with the [intermediate certificates post](/posts/what-makes-an-intermediate-certificate-intermediate/), inspecting the wikipedia.com certificate chain with `openssl s_client`:
...
I copied the PEM blocks into files named *end-entity* and *intermediate*:
...
```
Note that [overview section of rfc9525](https://datatracker.ietf.org/doc/html/rfc9525#name-overview-of-recommendations) states that **only** the DNS names in the subjectAltName extension field are to be used.  Historically the DN within the subject field of the certificate held the service identifier name - Many clients still accept a DNS name in the subject field for backwards compatibility.  [Appendix A. Changes from RFC 6125](https://datatracker.ietf.org/doc/html/rfc9525#name-changes-from-rfc-6125) clarify that this behaviour was updated as of rfc9525.
...
`openssl s_client` can also check the subjectAltName field using `-verify_hostname`:
...
Beyond subjectAltName, other fields and extensions contribute to the client's assessment of trust in the identity:
...
Other attributes also play a role in client verification of identity and validity:
...
This covers the end-to-end model of service identity verification and the key extensions a client uses to determine validity and trust. One nuance not covered here is the distinction between DV and EV certificates, which affects the degree of assurance the model provides.

> This [section](https://www.feistyduck.com/library/openssl-cookbook/online/openssl-command-line/examining-public-certificates.html) of the [openssl cookbook](https://www.feistyduck.com/library/openssl-cookbook/) (a free companion to the excellent [bulletproof TLS and PKI](https://www.feistyduck.com/books/bulletproof-tls-and-pki/) book) covers similar ground with further detail on the anatomy of x509 public certificates.
{: .prompt-info }